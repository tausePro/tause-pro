...
@section('titlebar_actions')
    <div class="flex gap-4 lg:justify-end">
        ...
        <x-button id="brand_autofill_btn" variant="secondary">
            <x-tabler-wand class="size-4" />
            {{ __('Autocompletar desde sitio') }}
        </x-button>
    </div>
@endsection
...
@section('settings')
    ...
    >
        ...
        <x-form-step
            step="1.1"
            label="{{ __('Deep Brand Context (Optional)') }}"
        />

        <x-forms.input
            id="uvps"
            label="{{ __('Unique Value Propositions (UVPs)') }}"
            tooltip="{{ __('List the main value propositions that make your brand different.') }}"
            type="textarea"
            rows="3"
            name="uvps"
        />
        ...
        <x-forms.input
            id="objections"
            label="{{ __('Frequent Objections') }}"
            tooltip="{{ __('Common customer objections when evaluating your product/service.') }}"
            type="textarea"
            rows="3"
            name="objections"
        />
        ...
        <x-forms.input
            id="answers"
            label="{{ __('Standard Answers to Objections') }}"
            tooltip="{{ __('Preferred replies or rebuttals to the common objections.') }}"
            type="textarea"
            rows="3"
            name="answers"
        />
        ...
        <x-forms.input
            id="competitors"
            label="{{ __('Main Competitors') }}"
            tooltip="{{ __('Comma-separated competitors or alternatives in your market.') }}"
            name="competitors"
            placeholder="{{ __('e.g. Competidor A, Competidor B, Competidor C') }}"
        />

        <x-forms.input
            id="content_pillars"
            label="{{ __('Content Pillars') }}"
            tooltip="{{ __('Foundational themes for your content strategy (comma-separated or multiline).') }}"
            type="textarea"
            rows="3"
        ...
        <x-forms.input
            id="preferred_ctas"
            label="{{ __('Preferred CTAs') }}"
            tooltip="{{ __('Calls-to-action you prefer to use (comma-separated).') }}"
            name="preferred_ctas"
            placeholder="{{ __('e.g. Agenda tu demo, Cotiza ahora, Compra aquí') }}"
        />

        <x-forms.input
            id="priority_channels"
            label="{{ __('Priority Channels') }}"
            tooltip="{{ __('Primary communication or distribution channels (comma-separated).') }}"
            name="priority_channels"
            placeholder="{{ __('e.g. Instagram, WhatsApp, Email, YouTube') }}"
        />

        <x-forms.input
            id="tone_restrictions"
            label="{{ __('Tone Restrictions or Compliance Notes') }}"
            tooltip="{{ __('Words/claims to avoid or regulatory constraints for the brand.') }}"
            type="textarea"
            rows="3"
            name="tone_restrictions"
        />
...
@push('script')
    <script>
        ...
        $('#custom_company_button').click(function() {
            ...
            formData.append('uvps', $("#uvps").val());
            formData.append('objections', $("#objections").val());
            formData.append('answers', $("#answers").val());
            formData.append('competitors', $("#competitors").val());
            formData.append('content_pillars', $("#content_pillars").val());
            formData.append('preferred_ctas', $("#preferred_ctas").val());
            formData.append('priority_channels', $("#priority_channels").val());
            formData.append('tone_restrictions', $("#tone_restrictions").val());
        ...
        // Autocompletar desde sitio: no persiste; trae payload y rellena campos
        $('#brand_autofill_btn').on('click', function() {
            const website = $('#c_website').val();
            if (!website) { return toastr.error('Ingresa primero la URL del sitio'); }
            const formData = new FormData();
            formData.append('prefill', '1');
            formData.append('c_website', website);
            formData.append('_method', $('[name="_method"]').val());
            $.ajax({
                type: 'post',
                url: '{{ $action }}',
                ...
                success: function(resp) {
                    const p = resp.prefill || {};
                    if (p.name) $('#c_name').val(p.name);
                    if (p.tagline) $('#c_tagline').val(p.tagline);
                    if (p.description) $('#c_description').val(p.description);
                    if (p.brand_color) $('#c_color').val(p.brand_color);
                    if (p.tone_of_voice) $('#tone_of_voice').val(p.tone_of_voice);
                    if (p.target_audience) $('#target_audience').val(p.target_audience);
                    if (p.content_pillars) $('#content_pillars').val(p.content_pillars);
                    if (p.preferred_ctas) $('#preferred_ctas').val(p.preferred_ctas);
                    if (p.competitors) $('#competitors').val(p.competitors);
                    toastr.success('{{ __('Prefill completed') }}');
                },
                error: function(err) {
                    toastr.error('No fue posible autocompletar con esa URL');
                }
            });
        });
    </script>
