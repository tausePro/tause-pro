...
@foreach ($messages ?? [] as $message)
    ...
    <div
        class="lqd-chat-ai-bubble mb-2.5 flex max-w-full content-start items-start gap-2 last:mb-0"
                ...
                @if ($has_canvas_content)
                    @if ($is_chat_pro)
                        <div class="mb-3 w-full">
                            ...
                            >
                                <span
                                    class="inline-grid size-9 place-items-center rounded-full border-none bg-surface-background p-0 text-foreground shadow-lg shadow-black/5 transition-all group-hover/btn:scale-110 group-[&.loading]:scale-90 group-[&.streaming-on]:scale-90"
                                >
                    ...
                    <pre
                        class="chat-content is-html prose relative w-full max-w-none !whitespace-pre-wrap px-6 py-3.5 indent-0 font-[inherit] text-xs font-normal text-current [word-break:break-word] empty:hidden [&_*]:text-current">{!! $output !!}</pre>
                @else
                    <pre
