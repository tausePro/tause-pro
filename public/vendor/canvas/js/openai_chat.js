...
function updateChatButtons() {
	...
	const generate = async ev => {
		...
		const createInCanvas = Alpine?.store( 'canvasButtonStatus' )?.status;
		...
		const aiBubbleTemplate = document.querySelector( '#canvas_chat_ai_bubble' ).content.cloneNode( true );
		...
		aiBubbleTemplate.querySelector( '.canvas-tiptap-content' )?.classList.add( 'pointer-events-none' );
		if ( !createInCanvas ) {
			aiBubbleTemplate.querySelector( '.canvas-head-btns' )?.remove();
		} else {
			aiBubbleTemplate.querySelector( '.canvas-head-btns' )?.classList.add( '!hidden' );
		}
		...
		const aiChatAlpineData = aiBubbleTemplate.querySelector( '[x-data]' );
		...
		// add x-data for new chats
		async function addAlpineDataOnNewChat( message_id ) {
			lastAiChatBubble?.classList.remove( 'loading' );
			lastAiChatBubble.classList.remove( 'animating-words' );
			switchGenerateButtonsStatus( false );

			// ai new response
			aiChatAlpineData._x_dataStack[ 0 ].tiptapContent.message_id = message_id;
			const delay = time => new Promise( ( resolve ) => { setTimeout( resolve, time ) } )

			const responseText = getAiResponseString();
			const letterArray = responseText?.split( '' );
			for ( const entity of letterArray ) {
				aiChatAlpineData._x_dataStack[ 0 ].tiptapContent.content += entity;
				await delay( 10 );
			}
			aiChatAlpineData._x_dataStack[ 0 ].tiptapContent.content = responseText;

			if ( createInCanvas ) {
				lastAiChatBubble.querySelector( '.canvas-head-btns' )?.classList.remove( '!hidden' );
				lastAiChatBubble.querySelector( '.canvas-tiptap-content' )?.classList.remove( 'pointer-events-none' );
				lastAiChatBubble.querySelector( '.chat-content-container' )?.classList.add( 'flex', 'flex-col', 'p-4', 'border', 'gap-2.5', 'shadow-md' );

				try {
					const res = fetch( '/tiptap-content-store', {
						method: 'post',
						headers: {
			...
			fetchEventSource( '/dashboard/user/generator/generate-stream', {
				...
				onmessage: e => {
					...
					if ( e.data === '[DONE]' ) {
						...
						// Handle Alpine data for new chats
						addAlpineDataOnNewChat( streamed_message_id );
					}
			...
			try {
				...
				if ( signal.aborted ) {
					aiBubbleWrapper.querySelector( '.chat-content' ).innerHTML =
						'Request aborted by user. Not saved.';
				} else {
					switch ( error.status ) {
						case 429:
...
function changeChatTitle( streamed_message_id ) {
	const $lqdChatUserBubblesLength = document.querySelectorAll(
