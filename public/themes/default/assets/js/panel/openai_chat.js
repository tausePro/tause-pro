...
function updateChatButtons() {
	...
	const generate = async ev => {
		...
		if ( canvasModeActivated && canvasEditBtnTemplate ) {
			...
			aiBubbleWrapper.querySelector( '.chat-content-container' )?.insertAdjacentElement('afterbegin', canvasEditBtn.firstElementChild);
		}
				...
				onmessage: async e => {
					if ( !receivedMessageId ) {
						...
						if ( eventData.type === 'message' ) {
							...
							aiBubbleWrapper.setAttribute('data-message-id', streamed_message_id);
						}
					...
					if ( e.data === '[DONE]' ) {
						...
						if ( canvasModeActivated ) {
							try {
								const responseString = getAiResponseString();
								const formattedResponse = formatString( responseString );
...
function openChatAreaContainer( chat_id, website_url = null ) {
	...
	$.ajax( {
		...
		success: function ( data ) {
			...
			// Restaurar Brand Voice desde localStorage por categoría
			try {
				...
				if (categoryId) {
					...
					if (raw) {
						...
						if (sel?.companyName && badge && badgeText) {
							badgeText.textContent = sel.companyName;
							badge.classList.remove('hidden');
						}
						// Precargar selects si existen
						if (sel?.brandId) {
							const brandSelect = document.getElementById('chat_brand_voice');
							if (brandSelect) {
								brandSelect.value = sel.brandId;
							}
						}
					}
				}
