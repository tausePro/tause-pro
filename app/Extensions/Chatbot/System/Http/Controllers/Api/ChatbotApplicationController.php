...
class ChatbotApplicationController extends Controller
{
    ...
    public function storeMessage(ChatbotHistoryStoreRequest $request, Chatbot $chatbot, string $sessionId, ChatbotConversation $chatbotConversation): ChatbotHistoryResource
    {
        ...
        // Inyectar URLs correctas desde embeddings de website
        $response = $this->injectProductUrlsFromEmbeddings($chatbot, $response);
        }
    ...
    protected function injectProductUrlsFromEmbeddings(Chatbot $chatbot, string $response): string
    {
        // Obtener embeddings de website y product con URLs
        ...
        if ($embeddings->isEmpty()) {
            Log::debug('InjectUrls: No embeddings found', ['chatbot_id' => $chatbot->id]);
            return $response;
        }

        Log::debug('InjectUrls: Processing', [
            'chatbot_id' => $chatbot->id,
        ...
        // Crear un mapa de palabras clave a URLs para matching más flexible
        $urlMap = [];
        foreach ($embeddings as $embedding) {
            $title = trim($embedding->title);
            $correctUrl = trim($embedding->url);
            
            if (empty($title) || empty($correctUrl)) {
                continue;
            }

            // Limpiar el título removiendo prefijos comunes
            $cleanTitle = preg_replace('/^Producto:\s*/i', '', $title);
            $cleanTitle = preg_replace('/\s*-\s*Aliviate\s*$/i', '', $cleanTitle);
            $cleanTitle = preg_replace('/\s*-\s*[^-]+\s*$/i', '', $cleanTitle);
            $cleanTitle = trim($cleanTitle);
            
            // Extraer palabras clave significativas (más de 3 caracteres)
            $words = preg_split('/\s+/', strtoupper($cleanTitle));
            $keywords = array_filter($words, function($word) {
                return strlen($word) > 3 && !in_array($word, ['PARA', 'CON', 'SIN', 'LOS', 'LAS', 'UNA', 'UNO', 'DEL', 'DE']);
            });
            
            if (!empty($keywords)) {
                // Usar array de keywords en lugar de pattern regex para evitar errores
                // Reindexar array después del filter
        ...
        Log::debug('InjectUrls: URL Map created', ['total_mappings' => count($urlMap), 'first_mapping' => $urlMap[0] ?? null]);

        // Función helper para normalizar texto (remover acentos)
        ...
        // Normalizar la respuesta completa para búsqueda de contexto
        $responseNormalized = $normalizeText($response);

        // Buscar todos los links markdown en la respuesta
        $response = preg_replace_callback('/\[([^\]]+)\]\(([^)]+)\)/', function ($matches) use ($urlMap, &$replacements, $normalizeText, $responseNormalized) {
            $linkText = $matches[1];
            $currentUrl = trim($matches[2]);
            $linkTextNormalized = $normalizeText($linkText);
            
            Log::debug('InjectUrls: Found link', [
                'link_text' => $linkText,
                'current_url' => $currentUrl,
                'link_text_normalized' => $linkTextNormalized
            ]);
            
            // Verificar si la URL es del dominio de aliviate (necesita corrección)
            $isAliviateUrl = preg_match('/aliviate\.com\.co/i', $currentUrl);
            
            if (!$isAliviateUrl) {
                return $matches[0]; // No es del dominio, mantener original
            }
            
            // Buscar en el contexto completo de la respuesta (no solo en el texto del link)
            foreach ($urlMap as $mapping) {
                // Verificar si las keywords del producto están presentes en la RESPUESTA COMPLETA
                ...
                if ($allKeywordsMatch) {
                    // Las keywords del producto están en la respuesta
                    // Verificar si la URL actual es la correcta
                    $isCorrectUrl = ($currentUrl === $mapping['url'] || 'https://' . parse_url($currentUrl, PHP_URL_HOST) . parse_url($currentUrl, PHP_URL_PATH) === $mapping['url']);
                    
                    if (!$isCorrectUrl) {
                        Log::debug('InjectUrls: Replacing URL (context-based match)', [
                            'from' => $currentUrl,
                            'to' => $mapping['url'],
                            'link_text' => $linkText,
                            'matched_title' => $mapping['title'],
                            'keywords' => implode(', ', $mapping['keywords']),
                            'method' => 'context_match'
                        ]);
                        $replacements++;
                        return "[{$linkText}]({$mapping['url']})";
                    }
            ...
            return $matches[0]; // Mantener original si no hay coincidencia
        }, $response);

        Log::debug('InjectUrls: Completed', ['replacements' => $replacements]);
