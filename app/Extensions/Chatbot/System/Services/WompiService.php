...
class WompiService
{
    ...
    public function generateCartPaymentLink(
        ...
        try {
            ...
            // Preparar payload
            $payload = [
                'name' => 'Compra - ' . $chatbot->title,
                'description' => implode(', ', $description),
                'single_use' => false,
                'collect_shipping' => true, // Para carrito, recoger dirección
                'currency' => 'COP',
                'amount_in_cents' => (int) $amount,
                'redirect_url' => $this->getRedirectUrl($chatbot),
                'expires_at' => now()->addHours(24)->toIso8601String(),
            ...
            return [
                'success' => true,
                'payment_link' => $data['permalink'] ?? $data['url'] ?? null,
                'reference' => $this->generateReference($chatbot),
                'amount' => $total,
                'items' => count($items),
                'expires_at' => $payload['expires_at'],
                'data' => $data,
