...
>
    ...
    <div class="flex flex-col gap-7 pt-9">
        ...
        <div>
            <h3 class="mb-5 text-sm font-semibold text-heading-foreground">
                @lang('Advanced Customization')
            </h3>
            ...
            <div class="mb-5">
                <x-forms.input
                    ...
                    placeholder="{{ __('https://example.com/background.jpg') }}"
                    name="custom_bg_image_url"
                    size="lg"
                    x-model="activeChatbot.custom_bg_image_url"
                    @input.throttle.250ms="externalChatbot && externalChatbot.toggleWindowState('open')"
                />
                <p class="mt-1 text-2xs opacity-60">
                    @lang('Upload a custom background image for the chat window')
                </p>
                <template
                    x-for="(error, index) in formErrors.custom_bg_image_url"
                    :key="'error-' + index"
                >
                    <div class="mt-2 text-2xs/5 font-medium text-red-500">
                        <p x-text="error"></p>
            ...
            {{-- Customize Texts --}}
            <div class="space-y-4">
                <h4 class="text-xs font-medium text-heading-foreground">
                    @lang('Customize Texts')
                </h4>
                ...
                <div>
                    <x-forms.input
                        ...
                        placeholder="{{ __('Hi there! How can we help you?') }}"
                        name="welcome_text"
                        size="lg"
                        x-model="activeChatbot.welcome_text"
                        @input.throttle.250ms="if ( externalChatbot ) { externalChatbot.toggleWindowState('open'); externalChatbot.toggleView('welcome') }"
                    />
                    <template
                        x-for="(error, index) in formErrors.welcome_text"
                        :key="'error-' + index"
                    >
                        <div class="mt-2 text-2xs/5 font-medium text-red-500">
                            <p x-text="error"></p>
                ...
                <div>
                    <x-forms.input
                        class:label="text-heading-foreground"
                        label="{{ __('Initial Prompt Text') }}"
                        placeholder="{{ __('Ask me anything. We usually reply in a few hours.') }}"
                        name="initial_prompt_text"
                        size="lg"
                        x-model="activeChatbot.initial_prompt_text"
                        @input.throttle.250ms="externalChatbot && externalChatbot.toggleWindowState('open')"
                    />
                    <template
                        x-for="(error, index) in formErrors.initial_prompt_text"
                        :key="'error-' + index"
                    >
                        <div class="mt-2 text-2xs/5 font-medium text-red-500">
                            <p x-text="error"></p>
                ...
                <div>
                    <x-forms.input
                        ...
                        size="lg"
                        x-model="activeChatbot.cta_button_text"
                        @input.throttle.250ms="externalChatbot && externalChatbot.toggleWindowState('open')"
                    />
                    <template
                        x-for="(error, index) in formErrors.cta_button_text"
                        :key="'error-' + index"
                    >
                        <div class="mt-2 text-2xs/5 font-medium text-red-500">
                            <p x-text="error"></p>
