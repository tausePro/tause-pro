...
>
    <form
        ...
        action="{{ route('dashboard.user.marketing-bot.train.qa') }}"
        @submit.prevent="ev => addText(ev, 'qa')"
    >
        <x-form-step
            step="1"
            label="{{ __('Add Q/A') }}"
    ...
    <form
        ...
        action="{{ route('dashboard.user.marketing-bot.train.generate.embedding') }}"
    ...
    >
        ...
        <div class="space-y-4">
            <template x-for="embedding in embeddings.filter(e => e.type === 'qa')">
                <div
                ...
                >
                    <x-forms.input
                        ...
                        data-type="qa"
                        ::id="'train-qa-item-' + embedding.id"
                        name="embedding-item"
                        type="checkbox"
                        ::value="embedding.id"
                        ::checked="embedding.status === 'Trained'"
                        x-init="$el.closest('label').setAttribute('for', 'train-qa-item-' + embedding.id);"
                        custom
                    >
                        <x-slot:label>
