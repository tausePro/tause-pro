...
class BrandController extends Controller
{
    ...
    public function store(BrandRequest $request): JsonResponse
    {
        ...
        // Modo autocompletar: no persiste, retorna payload para poblar el formulario
        if ($request->boolean('prefill')) {
            $data = $this->service->prefillFromWebsite((string) $request->get('website'));
            return response()->json([
                'message' => __('Prefill generated successfully.'),
                'type'    => 'success',
                'prefill' => $data,
            ]);
        }
