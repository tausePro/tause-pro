...
class AIController extends Controller
{
    ...
    public function buildOutput(Request $request)
    {
        ...
        if ($request->company) {
            ...
            if ($company) {
                // If no explicit tone provided, fallback to company's tone of voice
                if (empty($request->tone_of_voice) && empty($request->tone_of_voice_custom) && !empty($company->tone_of_voice)) {
    ...
    public function imageOutput($param, $post, $user): JsonResponse
    {
        ...
        $engineCheck = match ($param['image_generator']) {
            'flux-pro', 'ideogram', 'flux-pro-kontext' => EngineEnum::FAL_AI->value,
